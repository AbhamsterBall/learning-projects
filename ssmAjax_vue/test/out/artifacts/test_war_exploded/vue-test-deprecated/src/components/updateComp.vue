...
export default {
  ...
  methods: {
    async getData() {
      const response = await axios.get('http://localhost:8080/entity/' + this.$route.params.pk);
    ...
    async updateData() {
      ...
      const response = await axios.get("http://localhost:8080/return/update", {params : params})
